...
public class Statistic {
    ...
    public String getSummary() {
        ...
        if (success > 0) {
            ...
            builder.append(" bytes").append(StringUtil.lineSeparator());
            builder.append("all: tx ").append(toAmount(txAll));
            builder.append(", rx ").append(toAmount(rxAll));
            builder.append(StringUtil.lineSeparator());
            if (lostResponses > 0) {
    ...
    private String toAmount(long bytes) {
        if (bytes < 10000) {
            return bytes + " bytes";
        } else if (bytes < 10000000) {
            double value = bytes / 1000D;
            return String.format("%,.2f K-bytes", value);
        } else {
            double  value = bytes / 1000000D;
            return String.format("%,.2f M-bytes", value);
        }
    }
